...
class KARSEIN(nn.Module):
    def __init__(self, feature_dict,vocab_size_dict, bit_width, vec_width, grid=3, k=3, noise_scale=0.1, noise_scale_base=0.1, base_fun=torch.nn.SiLU, grid_eps=1.0, grid_range=[-1,1],scale_spline=1.0,use_bit_wise=True,use_vec_wise=True,emb_dim=16,pairwise_multiplication=[0,1]):
        ...
        self.sparse2emb_embeddings_bit = nn.ModuleDict({
            name: nn.Embedding(num_embeddings=self.vocab_size_dict[name], embedding_dim=self.emb_dim)
            for name in self.sparse_feature
        })
        
        self.varsparse2emb_embeddings_bit = nn.ModuleDict({
            name: nn.Embedding(num_embeddings=self.vocab_size_dict[name], embedding_dim=self.emb_dim)
            for name in self.varlen_sparse_feature
        })
        
        self.sparse2emb_embeddings_vec = nn.ModuleDict({
            name: nn.Embedding(num_embeddings=self.vocab_size_dict[name], embedding_dim=self.emb_dim)
            for name in self.sparse_feature
        })
        
        self.varsparse2emb_embeddings_vec = nn.ModuleDict({
            name: nn.Embedding(num_embeddings=self.vocab_size_dict[name], embedding_dim=self.emb_dim)
            for name in self.varlen_sparse_feature
        })
    ...
    def forward_bit(self, x):
        dense, sparse, varsparse = x

        # Efficiently process sparse2emb embeddings
        sparse2emb_list = [
            embedding(sparse[:, index].long())
            for index, embedding in enumerate(self.sparse2emb_embeddings_bit.values())
        ]
        sparse2emb = torch.cat(sparse2emb_list, dim=1)
        
        # Reshape the embeddings for further processing
        concatenated_emb = sparse2emb.view(-1, sum(e.embedding_dim for e in self.sparse2emb_embeddings_bit.values()))
        ...
        if self.varlen_sparse_feature:
            varsparse2emb_list = [
                ...
                for index, embedding in enumerate(self.varsparse2emb_embeddings_bit.values())
            ]
            ...
            varsparse2emb_reshaped = varsparse2emb_mean.view(-1, sum(e.embedding_dim for e in self.varsparse2emb_embeddings_bit.values()))
    ...
    def forward_vec(self, x):
        ...
        sparse2emb_list = [
            embedding(sparse[:, index].long())
            for index, embedding in enumerate(self.sparse2emb_embeddings_vec.values())
        ]
        sparse2emb = torch.cat(sparse2emb_list, dim=1)
        
        # Reshape the embeddings for further processing
        concatenated_emb = sparse2emb.view(-1, sum(e.embedding_dim for e in self.sparse2emb_embeddings_vec.values()))
        matrix_X1 = concatenated_emb.view(-1, len(self.sparse_feature), self.emb_dim)
        ...
        if self.varlen_sparse_feature:
            varsparse2emb_list = [
                embedding(varsparse[:, index].long())
                for index, embedding in enumerate(self.varsparse2emb_embeddings_vec.values())
            ]
            # Mean pooling across variable-length sparse embeddings
            varsparse2emb = torch.cat(varsparse2emb_list, dim=1)
            varsparse2emb_mean = torch.mean(varsparse2emb, dim=1)
            varsparse2emb_reshaped = varsparse2emb_mean.view(-1, sum(e.embedding_dim for e in self.varsparse2emb_embeddings_vec.values()))
    ...
    def forward(self,x):
        dense_bit, concatenated_emb_bit, matrix_X1_bit=self.forward_bit(x)
        dense_vec, concatenated_emb_vec, matrix_X1_vec=self.forward_vec(x)
        
        if self.use_vec_wise==True:
            matrix_output=matrix_X1_vec.clone()
            for i in range(len(self.karsein_vec_list)):
                if i in self.pairwise_multiplication:
                    crossed_features=self.cross_feature(matrix_X1_vec,matrix_output)
                    matrix_input=torch.cat([matrix_X1_vec,crossed_features],dim=1)
                else:
        ...
        if self.use_bit_wise==True and self.use_vec_wise==False:
            bit_input=torch.cat([concatenated_emb_bit,dense_bit],dim=1)
